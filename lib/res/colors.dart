import 'package:flutter/material.dart';

class AppColors {
  static Color get primary => const Color.fromRGBO(46, 96, 247, 1);

  static const Color secondary = Color(0xFFD8E6F8);
  static const Color secondaryLight = Color(0xFFEFF6FF);

  static Color get secondaryBlue => const Color.fromRGBO(216, 230, 248, 1);
  static Color get secondaryLightBlue => const Color.fromRGBO(239, 246, 255, 1);
  static Color get success => const Color.fromRGBO(28, 212, 35, 1);
  static Color get warning => const Color.fromRGBO(231, 34, 34, 1);
  static Color get black => const Color.fromRGBO(57, 56, 56, 1);
  static Color get placeholder => const Color.fromRGBO(73, 69, 79, 1);
}
